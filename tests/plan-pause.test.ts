...
import { createKnowledgeDemoData, ensureTodayKnowledgeTasks } from "../lib/knowledge/demo-store";
import {
...
describe("plan pause", () => {
  ...
  it("freezes first-cycle day, week, and daily task generation while paused", () => {
    ...
    expect(ensureTodayAlgorithmTasks(algorithm, algorithmProblems, day("2026-09-14"), {
      pausePeriods: pauses,
    })).toMatchObject({ data: algorithm, tasks: [], currentWeek: 1 });
    expect(ensureTodayKnowledgeTasks(knowledge, knowledgeQuestions, day("2026-09-14"), {
      pausePeriods: pauses,
    })).toMatchObject({ data: knowledge, tasks: [], currentWeek: 1 });
