...
- 并行开发任务（本 Agent）：执行优化路线第 3 项「八股 AI Recall 历史对比」；仅修改历史推导、页面和对应测试/文档，题库答案交其他 Agent。源码和用例已写入，Node 质量门、真实刷新与生产验收未运行。
...
- [ ] 保留历史、已有待办和 Mastery；暂停期间新到期复习顺延、同日暂停恢复幂等：已编写单测及 SQLite 测试，真实数据库并发、异常和跨时区回归尚待运行。
